...
- Actionable feedback means:
  ...
  - unresolved inline review threads, fetched via `gh api graphql` — split `$BATON_ISSUE_REPO`
    ...
    request fields for the thread root comment and an initial thread-comment page such as
    `comments(first:10)`, then paginate further as needed until you can identify the latest
    comment without the `<!-- baton-agent-reply -->` marker
