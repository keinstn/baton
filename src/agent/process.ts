 ...
 *
 * NOTE: cmd.exe expands %VAR% sequences in all arguments it processes. This
 * affects the Copilot adapter's -p prompt argv when prompts contain %-patterns.
 * ClaudeCode prompts are delivered via stdin and are not affected.
 */
function resolveWindowsCommand(executable: string): [string, string[]] {
...
export function runSubprocess(
  ...
  return new Promise((resolvePromise) => {
    ...
    if (typeof opts.command === "string") {
      ...
      if (isWindows) {
        const [resolved, prefix] = resolveWindowsCommand(first);
        executable = resolved;
        execArgs = [...prefix, ...rest];
      } else {
        executable = first;
        execArgs = rest;
      }
