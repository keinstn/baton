...
describe("startHttpServer (SPEC §13.7)", () => {
  ...
  describe("GET /api/v1/<identifier>", () => {
    ...
    it("resolves owner__repo-N identifiers produced by the tracker", async () => {
      const srv = await startTestServer(
        makeSnapshot({
          running: [makeRunning({ identifier: "acme__my-repo-1" })],
        }),
      );
      try {
        const res = await fetch(`${srv.baseUrl}/api/v1/acme__my-repo-1`);
        expect(res.status).toBe(200);
        const body = (await res.json()) as { identifier: string };
        expect(body.identifier).toBe("acme__my-repo-1");
      } finally {
        await srv.close();
      }
    });
